...
class GreedyAddRemoveKCenterSolver(KCPSolver):
    def __init__(self):
        self._name = "Greedy Add Remove K-Center"
    ...
    def solve(self, runNum=None):
        ...
        while True:
            ...
            candidate_add_nodes = self._get_farthest_nonselected_clients(selected_facilities, top_m=3)
            if len(candidate_add_nodes) == 0:
                break

            best_add_node = None
            best_remove_node = None
            best_radius = current_radius

            for add_node in candidate_add_nodes:
                self._facilities[0, add_node] = 1

                for remove_node in selected_facilities:
                    if remove_node == add_node:
                        continue
            ...
            if best_add_node is None or best_remove_node is None:
                break

            self._facilities[0, best_remove_node] = 0
            self._facilities[0, best_add_node] = 1
            self._ensure_exactly_k_facilities()

            accepted_facilities, accepted_radius = self._calculate_facilities_and_distance()
            self._assert_valid_facility_count(accepted_facilities)

            if accepted_radius >= current_radius:
                self._facilities[0, best_add_node] = 0
                self._facilities[0, best_remove_node] = 1
    ...
    def _get_farthest_nonselected_clients(self, selected_facilities, top_m=3):
        ...
        candidate_count = min(top_m, self._n - len(selected_facilities))
        if candidate_count <= 0:
            return []

        top_indices = torch.topk(candidate_scores, k=candidate_count).indices.tolist()
        return [index for index in top_indices if index not in selected_facilities]
